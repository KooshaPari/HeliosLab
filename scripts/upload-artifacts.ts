...
async function main() {
	...
	if (!artifactsDir) {
		console.error(
			"❌ Usage: bun run scripts/upload-artifacts.ts -- <artifactsDir>",
		);
		process.exit(1);
	}
	...
	console.log(
		`🚀 Uploading artifacts from ${artifactsDir} to bucket ${bucket}`,
	);
	...
	console.log(
		`➡️  Uploading ${files.length} files with concurrency ${concurrency}`,
	);
