...
function _normalizeChangedFiles(files: string[]): ChangedFile[] {
...
// Test-support dirs (fixtures, spawned helper entry points), never source
// needing a paired test. Matches at a path's start as well as after a
// separator; a bare "tests/" is excluded because source lives there.
function isTestTree(p: string): boolean {
	return /(?:^|\/)(fixtures|__fixtures__|__tests__)\//.test(p);
}
...
function getLineLimit(filePath: string): number {
	...
	for (const [configPath, limit] of Object.entries(FILE_LINE_LIMITS)) {
		...
		if (
			normalizedPath.endsWith(normalizedConfig) ||
			normalizedPath === normalizedConfig
		) {
			return limit;
		}
	}
			...
			lines.forEach((line: string, index: number) => {
				...
				if (
					...
					findings.push({
						...
						remediationHint:
							"Replace with specific type or use `unknown` with type guard",
					});
				}
				...
				if (/(?:API_KEY|SECRET|PASSWORD|TOKEN)\s*=\s*["']/.test(line)) {
					...
					findings.push({
						...
						constitutionLine: sections.get(section) || 0,
						remediationHint:
							"Move to environment variables or secure config, never commit secrets",
					});
				}
			});
		} catch {
...
if (import.meta.main) {
	const argv =
		typeof Bun !== "undefined" ? Bun.argv : (globalThis.process?.argv ?? []);
