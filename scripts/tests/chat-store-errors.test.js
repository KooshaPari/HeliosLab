...
const source = readFileSync(new URL("../../apps/colab-renderer/src/stores/chat.store.ts", import.meta.url), "utf8");
...
for (const [label, error, message] of [
  ["API error", new AnthropicApiError(429, "rate limited", "request failed"), "Anthropic API error 429: rate limited"],
  ["ordinary error", new Error("offline"), "offline"],
  ["string rejection", "network unavailable", "network unavailable"],
  ["null rejection", null, "null"],
  ["invalid status", new AnthropicApiError("429", "body", "invalid status"), "invalid status"],
  ["invalid body", new AnthropicApiError(429, null, "invalid body"), "invalid body"],
]) {
  test(`chat finalizes a failed response for ${label}`, async () => {
    ...
    const api = {
      ...
      toAnthropicHistory: () => [],
      sendMessages: async () => { throw error; },
    };
    const exports = {};
    runInThisContext(`(function(require, exports) {${compiled}\n})`)(
      name => name === "@helios/runtime-core" ? api : require(name),
      exports,
    );
