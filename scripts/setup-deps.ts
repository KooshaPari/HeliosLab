...
async function vendorLlama() {
	...
	try {
		...
		execSync("cd llama-cli && chmod +x setup-llama.sh && ./setup-llama.sh", {
			stdio: "inherit",
		});
		...
		if (existsSync(cmakeListsPath)) {
			...
			console.error(
				"--------> Debug: llamaPath exists =",
				existsSync(llamaPath),
			);
			...
			throw new Error(
				"llama.cpp was not cloned properly - CMakeLists.txt not found",
			);
		}
	} catch (error) {
