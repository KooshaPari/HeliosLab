...
async function buildPtyBinary() {
	...
	const ptyBinaryMacOSTarget = path.join(
		APP_BUNDLE_FOLDER,
		"Contents",
		"MacOS",
		"colab-pty",
...
async function buildLlamaCli() {
	...
	try {
		...
		if (!existsSync(libLlamaPath)) {
			console.error(
				"--------> llama.cpp libraries not found at:",
				libLlamaPath,
			);
		...
		const buildOptionsPath = path.join(
			"llama-cli",
			"src",
			"build_options_default.zig",
		);
		...
		const llamaCliBinarySource = path.join(
			"llama-cli",
			"zig-out",
			"bin",
			"llama-cli",
...
async function buildMockLlamaCli() {
	...
	try {
		...
		if (existsSync(llamaCliBinarySource)) {
			...
			console.error(
				"--------> Mock llama-cli binary not found at:",
				llamaCliBinarySource,
			);
		}
