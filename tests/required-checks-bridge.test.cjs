...
const suite = (id = 1, status = "completed") => ({
	id,
	status,
	head_sha: sha,
	app: { id: 15368 },
...
async function audit(
	...
	const github = {
		rest: {
			...
			pulls: {
				get: async () => ({
					data: { head: { sha: heads[Math.min(reads++, heads.length - 1)] } },
				}),
			},
		},
		paginate: async (method, args) => {
			...
			const data =
				method === actions.listWorkflowRuns
					? executions
...
test("workflow grants no mutation permissions or checkout", () => {
	assert.doesNotMatch(
		workflow,
		/:\s*write|actions\/checkout|createCommitStatus/,
	);
});
test("all four successful app checks across multiple pages pass", async () => {
	const filler = Array.from({ length: 100 }, (_, id) => ({
		...good()[0],
		name: "unrelated",
		id: id + 100,
	}));
...
test("newer pending rerun supersedes old success", async () => {
	assert.equal(
		(await audit([...good(), { ...good()[0], id: 99, status: "queued" }]))
			.failures.length,
		1,
	);
});
...
test("pending or missing suite cannot pass", async () => {
	assert.equal(
		(await audit(good(), [suite(1, "in_progress")])).failures.length,
		1,
	);
...
test("new queued workflow suite without jobs supersedes old successful checks", async () => {
	...
	assert.equal(
		(await audit(good(), [suite(), suite(2, "queued")], [sha], executions))
			.failures.length,
		1,
	);
});
test("queued rerun attempt and missing workflow fail closed", async () => {
	...
	assert.equal(
		(await audit(good(), [suite()], [sha], executions)).failures.length,
		1,
	);
...
for (const conclusion of [
	...
	test(`completed workflow rerun with ${conclusion} cannot reuse green check records`, async () => {
		...
		assert.equal(
			(await audit(good(), [suite()], [sha], [execution])).failures.length,
			1,
		);
	});
}
