...
import type { AssetConnection, StorageManager } from "../storage/manager";
...
async function handleUpload(
	...
	if (args.length === 0) {
		write(
			"\x1b[31mUsage: wf assets upload <file> [--site <site_id>]\x1b[0m\r\n",
		);
		return;
	}
	...
	if (!siteId) {
		...
		write(
			"Use --site <site_id> or run from a directory with .webflowrc.json\r\n",
		);
		return;
	}
...
async function handleSync(
	...
	if (!siteId) {
		write(
			'\x1b[31mNo site configured.\x1b[0m Run "wf devlink init" first.\r\n',
		);
		return;
	}
...
async function handleInject(
	...
	write(
		`\x1b[36mGenerating custom code from asset connections...\x1b[0m\r\n\r\n`,
	);
	...
	if (assets.length === 0) {
		...
		write(
			'Use "wf assets connect" to configure where assets should be injected.\r\n',
		);
		return;
	}
	...
	const pageConnections: Map<string, { head: string[]; footer: string[] }> =
		new Map();
...
function printHelp(write: (text: string) => void): void {
	...
	write(
		'  4. Run "wf assets connect <file> --site-head" to configure injection\r\n',
	);
