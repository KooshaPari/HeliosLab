...
async function handleInit(
	...
	if (existsSync(configPath)) {
		write(
			"\x1b[33mDevLink is already initialized in this directory.\x1b[0m\r\n",
		);
...
async function handlePull(
	...
	write(
		`\x1b[36mSyncing components from ${config.siteName || config.siteId}...\x1b[0m\r\n\r\n`,
	);
...
async function handleStatus(
	...
	if (!existsSync(configPath)) {
		write(
			"\x1b[33mNo DevLink configuration found in this directory.\x1b[0m\r\n",
		);
	...
	if (existsSync(devlinkPath)) {
		...
		write(
			'\x1b[33mComponents directory not found. Run "wf devlink pull".\x1b[0m\r\n',
		);
	}
	...
	if (await client.isAuthenticated()) {
		try {
			...
			if (site.lastPublished) {
				write(
					`Last published: ${new Date(site.lastPublished).toLocaleString()}\r\n`,
				);
			}
		} catch (_e) {
...
function printHelp(write: (text: string) => void): void {
	...
	write(
		"DevLink syncs visual components from Webflow Designer to your codebase.\r\n",
	);
	write(
		"Components are exported as React code that you can use in your apps.\r\n\r\n",
	);

	write("\x1b[1mCommands:\x1b[0m\r\n");
	write("  wf devlink init [site_id]   Initialize DevLink for a site\r\n");
	write(
		"  wf devlink pull             Pull latest components from Webflow\r\n",
	);
