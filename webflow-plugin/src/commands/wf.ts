...
async function handleSitesCommand(
	...
	if (sites.length === 0) {
		write(
			"No sites found. Make sure your token has access to at least one site.\r\n",
		);
		return;
	}
...
function printHelp(write: (text: string) => void): void {
	...
	write(
		'\x1b[90mRun "wf <command> --help" for more info on a command.\x1b[0m\r\n',
	);
}
