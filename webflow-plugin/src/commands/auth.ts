...
const OAUTH_REDIRECT_URI =
	process.env.WEBFLOW_REDIRECT_URI || "http://localhost:3333/callback";
const OAUTH_SCOPES = [
	"sites:read",
	"sites:write",
	"pages:read",
...
async function handleLogin(
	...
	if (!OAUTH_CLIENT_ID) {
		...
		write(
			'2. Use "wf auth token <access_token>" with a personal access token\r\n\r\n',
		);
	...
	write(
		"After authorizing, you'll be redirected. The token will be captured automatically.\r\n",
	);
...
async function handleStatus(
	...
	if (auth.expiresAt) {
		...
		if (expiresIn > 0) {
			...
			write(
				'  \x1b[33mToken expired. Run "wf auth" to re-authenticate.\x1b[0m\r\n',
			);
		}
	}
	...
	try {
		...
		write(
			"  \x1b[31mFailed to verify token. It may be invalid or expired.\x1b[0m\r\n",
		);
	}
}
...
async function handleSetToken(
	...
	try {
		...
		if (!response.ok) {
			await storage.clearAuth();
			write(
				`\x1b[31mInvalid token. API returned: ${response.status}\x1b[0m\r\n`,
			);
			return;
		}
