...
async function handleInit(
	...
	write(
		`Scaffolding ${framework === "astro" ? "Astro" : "Next.js"} project...\r\n\r\n`,
	);
...
`;
	...
	writeFileSync(
		join(cwd, ".webflowrc.json"),
		JSON.stringify(webflowrc, null, 2),
	);
...
async function handleLogs(
	...
	try {
		...
		write(
			"\x1b[33mCould not fetch logs. Make sure wrangler is configured.\x1b[0m\r\n",
		);
	}
}
...
async function handleStatus(
	...
	if (!existsSync(colabConfigPath)) {
		write(
			"\x1b[33mNo Webflow Cloud configuration found in this directory.\x1b[0m\r\n",
		);
