...
import type { Disposable, PluginAPI } from "../../src/main/plugins/types";
...
export async function activate(api: PluginAPI): Promise<void> {
			...
			const checkForToken = (): string | null => {
				try {
					...
					if (fileExists) {
						...
						api.log.info(
							`File content (${content.length} chars): ${content.slice(0, 100)}`,
						);
						// Look for either WEBFLOW_SITE_API_TOKEN or WEBFLOW_WORKSPACE_API_TOKEN
						const match = content.match(
							/WEBFLOW_(?:SITE_API|WORKSPACE_API)_TOKEN="?([^"\n]+)"?/,
						);
						if (match) {
			...
			const poll = () => {
				...
				if (token) {
					...
					api.notifications.showInfo(
						"Successfully authenticated with Webflow!",
					);
					api.settings.postMessage({
	...
	const deployToCloudWithExpect = async (
		cwd: string,
		siteId: string,
		siteToken: string,
	) => {
			...
			api.shell
				...
				.then((result) => {
					...
					if (
						...
						api.notifications.showInfo(
							"Deployed to Webflow Cloud successfully!",
						);
						api.state.set("cloudDeployStatus", {
							status: "success",
							output: result.stdout,
	...
	const settingsDisposable = api.settings.registerSchema({
		title: "Webflow",
		description:
			"Connect your Webflow account to sync components, deploy apps, and manage assets",
	...
	const initComponentsMenuDisposable = api.contextMenu.registerItem(
		...
		async (ctx) => {
			...
			const folderName = api.utils.getUniqueNewName(
				parentDir,
				"code-components",
			);
			const targetDir = join(parentDir, folderName);
			...
			const _workspaceToken = tokens.find(
				(t) =>
					t.status === "valid" &&
					(t.type === "oauth" || t.type === "workspace"),
			);
...
`;
			...
			packageJson.scripts = {
				...
				"webflow:bundle":
					"webflow library bundle --public-path http://localhost:4000/",
			...
			try {
				...
				if (installResult.status !== 0) {
					api.log.warn(
						`bun install exited with status ${installResult.status}`,
					);
					if (installResult.stderr) {
						api.log.warn(
							`bun install stderr: ${installResult.stderr.toString()}`,
						);
					}
				}
			} catch (err) {
...
`;
			...
			api.notifications.showInfo(
				`Webflow Cloud app created! Click on webflow.json to configure.`,
			);
	...
	// Register commands
	const initCmdDisposable = api.commands.registerCommand(
		"webflow.init",
		async () => {
			api.notifications.showInfo(
				'Use "wf init" in terminal to initialize a Webflow project',
			);
		},
	);
	disposables.push(initCmdDisposable);

	const syncCmdDisposable = api.commands.registerCommand(
		"webflow.sync",
		async () => {
			api.notifications.showInfo(
				'Use "wf devlink pull" in terminal to sync components',
			);
		},
	);
	disposables.push(syncCmdDisposable);

	const shareCmdDisposable = api.commands.registerCommand(
		"webflow.share",
		async () => {
			api.notifications.showInfo(
				'Use "wf components share" in terminal to share your library',
			);
		},
	);
	disposables.push(shareCmdDisposable);

	const deployCmdDisposable = api.commands.registerCommand(
		"webflow.deploy",
		async () => {
	...
	const uploadCmdDisposable = api.commands.registerCommand(
		"webflow.uploadAssets",
		async () => {
			api.notifications.showInfo(
				'Use "wf assets upload" in terminal to upload assets',
			);
		},
	);
