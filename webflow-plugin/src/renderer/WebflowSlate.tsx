...
export const WebflowSlate = (props: WebflowSlateProps): JSXElement => {
	...
	const [_pullRunning, setPullRunning] = createSignal(false);
	const [_statusRunning, setStatusRunning] = createSignal(false);
	const [shareRunning, setShareRunning] = createSignal(false);
	const [_bundleRunning, setBundleRunning] = createSignal(false);
	const [_deployRunning, _setDeployRunning] = createSignal(false);
	const [_devRunning, _setDevRunning] = createSignal(false);
	const [_commandOutput, setCommandOutput] = createSignal<string | null>(null);
	...
	const loadSitesFromState = async () => {
		try {
			...
			console.log(
				"[WebflowSlate] sites from plugin state:",
				sitesFromState?.length,
			);
	...
	const _runCommandWithEnvAndInput = async (
		cmd: string,
		args: string[],
		env: Record<string, string>,
		input: string,
	...
	const _runWfCommand = async (subcommand: string, ...args: string[]) => {
		...
		try {
			...
			if (exists) {
				...
				try {
					...
					// Handle both old string format and new object format
					const lsOutput =
						typeof lsResult === "string" ? lsResult : lsResult?.stdout || "";
					if (lsOutput) {
	...
	const _shareLibrary = async () => {
		...
		if (!libraryId?.trim()) {
			setError(
				"Library ID is required. Please set a Library ID before sharing.",
			);
			return;
		}
		...
		try {
			...
			const pollForStatus = async () => {
				...
				try {
					...
					if (status?.timestamp && status.timestamp > startTime) {
						if (status.status === "running") {
							setCommandOutput(
								"Sharing library to Webflow...\nThis may take a moment.",
							);
	...
	const [devServerTerminalId, setDevServerTerminalId] = createSignal<
		string | null
	>(null);
	...
	const updateCloudConfigSite = async (site: WebflowSite, _token: string) => {
		if (!props.node?.path) {
...
const DevLinkSlateContent = (props: {
	...
	const startSync = () => {
		...
		if (terminalMode() === "sync" && terminalRef) {
			terminalRef.run(
				`${getEnvPrefix()}bunx @webflow/webflow-cli devlink sync`,
			);
		} else {
	...
	const currentSite = () =>
		props.sites.find((s) => s.id === props.config?.siteId);

	// Check if we need to show the site selection UI (no site selected yet)
	const needsSiteSelection = () =>
		props.connected && props.config && !props.config.siteId;
	...
	return (
		<div>
					...
					<Show when={props.sites.length === 0}>
						...
						>
							...
							<p style={{ margin: 0, "font-size": "12px" }}>
								If this takes too long, try refreshing or check your connection
								in Settings.
							</p>
						</div>
					</Show>
			...
			<Show when={props.connected && props.config?.siteId}>
				...
				>
					...
					<Show when={showSitePicker()}>
						...
						>
							...
							>
								<For
									each={props.sites.filter(
										(s) => s.id !== props.config?.siteId,
									)}
								>
									{(site) => (
										<button
				...
				>
					...
					<ConfigField
						...
						value={props.config?.componentsPath}
						mono
					/>
...
const CodeComponentsSlateContent = (props: {
	...
	const [terminalMode, setTerminalMode] = createSignal<
		"none" | "share" | "dev"
	>("none");
	let terminalRef: ColabTerminalElement | null = null;
			...
			<Show when={props.config}>
				...
				>
					...
					<EditableConfigField
						label="Description"
						value={
							props.config?.library?.description ||
							(props.config as any)?.description
						}
					...
					<div style={{ "margin-top": "12px" }}>
						...
						>
							Components (
							{(props.config?.library?.components || props.config?.components)
								?.length || 0}
							)
						</label>
						<div style={{ display: "flex", gap: "8px", "flex-wrap": "wrap" }}>
							<For
								each={
									props.config?.library?.components ||
				...
				>
					...
					<ActionButton
						...
						description={
							terminalMode() === "dev" ? "Running on :4000" : "Bundle & serve"
						}
						onClick={() =>
							terminalMode() === "dev" ? stopTerminal() : startDevServer()
						}
					...
					>
						...
						>
							...
							>
								<span
									style={{
										color: terminalMode() === "share" ? "#a855f7" : "#60a5fa",
									}}
								>
									{terminalMode() === "share" ? "↑" : "▶"}
								</span>
...
const CloudSlateContent = (props: {
	...
	const getMountPath = () =>
		props.config?.cloud?.mountPath || props.config?.mountPath;
	...
	const getSiteName = () =>
		props.config?.cloud?.siteName || props.config?.siteName;
		...
		<div>
			...
			>
				...
				<div style={{ flex: 1 }}>
					...
					>
						Deploy {props.config?.framework || "apps"} to Webflow's edge
						infrastructure
					</p>
				</div>
			...
			<Show when={props.connected && props.config}>
					...
					>
						...
						<Show when={props.sites.length === 0}>
							<p style={{ color: "#666", "font-size": "13px" }}>
								No sites found. Make sure your Webflow account has sites
								available.
							</p>
						</Show>
					</div>
								...
								<For each={props.sites}>
									{(site) => (
										<button
											...
											style={{
												background:
													site.id === getSiteId() ? "#3a3a3a" : "#1e1e1e",
												border:
													site.id === getSiteId()
														? "1px solid #4353ff"
														: "1px solid #333",
				...
				>
					<ActionButton
						...
						label="Deploy"
						description={
							getSiteId() ? "Push to Webflow Cloud" : "Select a site first"
						}
						onClick={startDeploy}
					...
					<ActionButton
						...
						label={terminalMode() === "dev" ? "Stop Server" : "Dev Server"}
						description={
							terminalMode() === "dev" ? "Running on :4321" : "Run locally"
						}
						onClick={() =>
							terminalMode() === "dev" ? stopTerminal() : startDevServer()
						}
					...
					>
						...
						>
							...
							>
								<span
									style={{
										color: terminalMode() === "deploy" ? "#4ade80" : "#60a5fa",
									}}
								>
									{terminalMode() === "deploy" ? "🚀" : "▶"}
								</span>
...
const _TerminalOutputPanel = (props: {
	output: string | null;
	error: string | null;
	...
	const handleTerminalOutput = (
		event: CustomEvent<{ terminalId: string; data: string }>,
	) => {
		const data = event.detail;
		if (
			props.streamTerminalId &&
			data.terminalId === props.streamTerminalId &&
			terminal
		) {
	...
	onMount(() => {
		if (
			containerRef &&
			(props.output || props.error || props.streamTerminalId)
		) {
			initTerminal();
		}
	...
	return (
		...
		>
			...
			>
				...
				<span
					style={{ "font-size": "12px", "font-weight": 500, color: "#fff" }}
				>
