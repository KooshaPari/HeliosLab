...
export class CheckpointReader {
	...
	async read(): Promise<Checkpoint | null> {
		...
		try {
			...
			if (!this.isValidCheckpoint(checkpoint)) {
				console.warn(
					"Checkpoint integrity or schema validation failed - trying backup",
				);
	...
	private async readBackup(): Promise<Checkpoint | null> {
		...
		try {
			...
			if (!this.isValidCheckpoint(checkpoint)) {
				console.warn(
					"Backup checkpoint integrity or schema validation failed - total loss",
				);
				return null;
			}
	...
	private isValidCheckpoint(checkpoint: unknown): checkpoint is Checkpoint {
		return (
			this.verifyChecksum(checkpoint) && validateCheckpoint(checkpoint).valid
		);
	}

	private verifyChecksum(checkpoint: unknown): checkpoint is Checkpoint {
		if (
			!isCheckpointCandidate(checkpoint) ||
...
function validateSession(session: unknown): ValidationError[] {
	...
	if (!sessionId) {
		errors.push(
			validationError(sessionId, "sessionId", "Expected a non-empty string"),
		);
	}
	...
	for (const field of [
		...
		if (typeof session[field] !== "string" || session[field].length === 0) {
			errors.push(
				validationError(sessionId, field, "Expected a non-empty string"),
			);
		}
	}
	...
	if (typeof scrollbackSnapshot !== "string") {
		errors.push(
			validationError(sessionId, "scrollbackSnapshot", "Expected a string"),
		);
	} else if (
		Buffer.byteLength(scrollbackSnapshot, "utf8") > MAX_SCROLLBACK_SIZE
	) {
		errors.push(
			validationError(
				sessionId,
				"scrollbackSnapshot",
				`Exceeds ${MAX_SCROLLBACK_SIZE} byte limit`,
			),
		);
	}
