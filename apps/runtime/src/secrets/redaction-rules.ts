...
import {
	assertRedactionPatternConsumesInput,
	type RedactionRule,
...
function validateRule(rule: unknown): RedactionRule {
	...
	if (
		...
		throw new Error(
			`Invalid redaction rule '${candidate.id}': category must be non-empty`,
		);
	}
	if (!(candidate.pattern instanceof RegExp)) {
		throw new Error(
			`Invalid redaction rule '${candidate.id}': pattern must be non-empty`,
		);
	}
	assertRedactionPatternConsumesInput(candidate.pattern);
	if (typeof candidate.description !== "string") {
		throw new Error(
			`Invalid redaction rule '${candidate.id}': description must be a string`,
		);
	}
	if (typeof candidate.enabled !== "boolean") {
		throw new Error(
			`Invalid redaction rule '${candidate.id}': enabled must be a boolean`,
		);
	}
	if (
...
function parsePersistedRule(value: unknown): RedactionRule {
	...
	if (typeof candidate.pattern !== "string") {
		throw new Error(
			"Invalid persisted redaction rule: pattern must be a string",
		);
	}
	...
	try {
		...
		throw new Error(
			`Invalid persisted redaction rule regex: ${(error as Error).message}`,
		);
	}
...
export class RedactionRuleManager {
	...
	async importRules(filePath: string): Promise<void> {
		...
		for (const rule of validated) {
			if (ids.has(rule.id)) {
				throw new Error(
					`Invalid redaction rules config: duplicate id '${rule.id}'`,
				);
			}
			ids.add(rule.id);
		}
