...
function validateMasterKey(value: unknown, source: string): Buffer {
	if (!Buffer.isBuffer(value) || value.length !== KEY_BYTES) {
		throw new Error(
			`Invalid ${source}: master key must be exactly ${KEY_BYTES} bytes`,
		);
	}
	return value;
}
...
function parseMasterKeyHex(value: string, source: string): Buffer {
	if (!/^[0-9a-fA-F]{64}$/.test(value)) {
		throw new Error(
			`Invalid ${source}: master key must contain exactly 64 hexadecimal characters`,
		);
	}
...
function validateEncryptedPayload(value: unknown): EncryptedPayload {
	...
	if (
		...
		throw new Error(
			"Invalid encrypted payload: fields do not match the persisted schema",
		);
	}
...
export class EncryptionService {
	...
	constructor(opts?: {
		masterKeyOverride?: () => Promise<Buffer>;
		keyPath?: string;
	}) {
