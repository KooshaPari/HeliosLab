...
describe("CredentialStore: store and retrieve", () => {
	...
	it("rejects malformed persisted encrypted payloads without coercion", async () => {
		...
		for (const payload of [
			...
			await expect(
				store.retrieve("providerA", "ws1", "malformed"),
	...
	it("removes temporary ciphertext when atomic replacement fails", async () => {
		...
		await expect(
			store.store("providerA", "ws1", "blocked", "secret"),
		).rejects.toThrow();

		expect(
			readdirSync(credentialDir).filter((name) =>
				name.startsWith("blocked.enc.tmp."),
			),
		).toEqual([]);
	});
	...
	it("rejects path segments that collapse credential scopes", async () => {
		await expect(store.store(".", "providerA", "key", "val")).rejects.toThrow(
			"path segment",
		);
		await expect(store.store("providerA", ".", "key", "val")).rejects.toThrow(
			"path segment",
	...
	it("rejects platform-aliased and control-character path segments", async () => {
		for (const providerId of [
			...
			await expect(
				store.store(providerId, "ws1", "key", "val"),
	...
	it("rejects non-string identifiers without coercion", async () => {
		await expect(
			store.store(null as unknown as string, "ws1", "key", "val"),
...
describe("CredentialStore: rotate preserves file permissions", () => {
	...
	it("preserves the previous credential when replacement encryption fails", async () => {
		...
		encryption.encrypt = (plaintext, providerSalt) => {
			if (failEncryption)
				return Promise.reject(new Error("injected encryption failure"));
			return encrypt(plaintext, providerSalt);
		};
		const transactionalStore = new CredentialStore({
			dataDir: tmpDir,
			encryption,
		});
		...
		expect(
			await transactionalStore.retrieve("providerA", "ws1", "rotKey"),
		).toBe("original");
	});
});
