...
const DEFAULT_PATTERNS_BY_ID = new Map(
	DEFAULT_PATTERNS.map((pattern) => [pattern.id, pattern]),
);
...
function parsePattern(value: unknown, index: number): ProtectedPathPattern {
	if (typeof value !== "object" || value === null || Array.isArray(value)) {
		throw new Error(
			`Invalid protected path pattern at index ${index}: expected an object`,
		);
	}
	...
	if (typeof candidate.id !== "string" || candidate.id.trim() === "") {
		throw new Error(
			`Invalid protected path pattern at index ${index}: id must be non-empty`,
		);
	}
	if (typeof candidate.pattern !== "string") {
		throw new Error(
			`Invalid protected path pattern '${candidate.id}': pattern must be a string`,
		);
	}
	...
	if (typeof candidate.enabled !== "boolean") {
		throw new Error(
			`Invalid protected path pattern '${candidate.id}': enabled must be a boolean`,
		);
	}
...
function assertCanonicalDefaultIdentity(pattern: ProtectedPathPattern): void {
	...
	if (defaultPattern === undefined) {
		if (pattern.isDefault) {
			throw new Error(
				`Invalid protected path config: unknown default '${pattern.id}'`,
			);
		}
		return;
	}
	if (
		...
		throw new Error(
			`Invalid protected path config: cannot redefine default '${pattern.id}'`,
		);
	}
}
	...
	async importPatterns(path: string): Promise<void> {
		...
		for (const pattern of validated) {
			...
			if (ids.has(pattern.id)) {
				throw new Error(
					`Invalid protected path config: duplicate id '${pattern.id}'`,
				);
			}
			ids.add(pattern.id);
		}
