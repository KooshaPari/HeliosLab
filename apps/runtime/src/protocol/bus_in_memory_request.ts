...
export async function handleInMemoryRequest(
  ...
  if (command.method) {
    ...
    if (command.method === "runtime.lane.create") {
      const correlationId = command.correlation_id;
      if (!correlationId) {
        return makeErrorResponse(
          command,
          "MISSING_CORRELATION_ID",
          "correlation_id is required for runtime.lane.create"
        );
      }
      ...
      ctx.publishLifecycleEvent("runtime.lane.create.started", command);
      ctx.publishLifecycleEvent("runtime.lane.created", command);
    ...
    if (command.method === "runtime.lane.attach") {
    ...
    if (command.method === "runtime.session.attach") {
      const correlationId = command.correlation_id;
      if (!correlationId) {
        return makeErrorResponse(
          command,
          "MISSING_CORRELATION_ID",
          "correlation_id is required for runtime.session.attach"
        );
      }
      ...
      ctx.publishLifecycleEvent("runtime.session.attach.started", command);

      if (forceError) {
        ctx.lifecycleProgress.get(correlationId)?.add("runtime.session.attach.failed");
        ctx.publishLifecycleEvent("runtime.session.attach.failed", command);
      ...
      ctx.publishLifecycleEvent("runtime.session.attached", command);
      if (isRestore) {
    ...
    if (command.method === "runtime.terminal.spawn") {
      const correlationId = command.correlation_id;
      if (!correlationId) {
        return makeErrorResponse(
          command,
          "MISSING_CORRELATION_ID",
          "correlation_id is required for runtime.terminal.spawn"
        );
      }
      const forceError = command.payload?.force_error === true;
      ...
      ctx.publishLifecycleEvent("runtime.terminal.spawn.started", command);

      if (forceError) {
        ctx.lifecycleProgress.get(correlationId)?.add("runtime.terminal.spawn.failed");
        ctx.publishLifecycleEvent("runtime.terminal.spawn.failed", command);
      ...
      ctx.publishLifecycleEvent("runtime.terminal.spawned", command);
    ...
    if (command.method === "runtime.terminal.input") {
      if (
    ...
    if (command.method === "runtime.renderer.capabilities") {
      return {
        id: `res-${Date.now()}`,
        type: "response",
        ts: new Date().toISOString(),
    ...
    if (command.method === "runtime.renderer.switch") {
