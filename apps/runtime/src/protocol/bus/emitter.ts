...
export class InMemoryLocalBus implements LocalBus {
  ...
  async request(command: LocalBusEnvelope): Promise<LocalBusEnvelope> {
    ...
    if (command.method) {
      ...
      if (command.method === "runtime.lane.create") return handleLaneCreate(command, startTime, ctx);
      if (command.method === "runtime.lane.attach") return handleLaneAttach(command, ctx);
      if (command.method === "runtime.session.attach") return handleSessionAttach(command, startTime, ctx);
      if (command.method === "runtime.terminal.spawn") return handleTerminalSpawn(command, startTime, ctx);
      if (command.method === "runtime.terminal.input") return handleTerminalInput(command);
      if (command.method === "runtime.renderer.capabilities")
        return handleRendererCapabilities(this.rendererEngine);
      if (command.method === "runtime.renderer.switch") return handleRendererSwitch(command, ctx);
    }
