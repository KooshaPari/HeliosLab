...
export function handleLaneAttach(
  ...
  ctx.lifecycleProgress.get(correlationId)?.add("runtime.lane.attach.started");
  publishLifecycleEvent("runtime.lane.attach.started", command, ctx.eventLog, ctx.auditLog);

  const laneId = command.lane_id ?? command.payload?.lane_id ?? `lane_${Date.now()}`;

  ctx.lifecycleProgress.get(correlationId)?.add("runtime.lane.attached");
  publishLifecycleEvent("runtime.lane.attached", command, ctx.eventLog, ctx.auditLog);
...
export function handleLaneCreate(
  ...
  if (!correlationId) {
    return {
      ...
      error: {
        ...
        message: "correlation_id is required for runtime.lane.create",
        retryable: false,
      },
    };
  }
  ...
  ctx.lifecycleProgress.get(correlationId)?.add("runtime.lane.create.started");
  publishLifecycleEvent("runtime.lane.create.started", command, ctx.eventLog, ctx.auditLog);
  publishLifecycleEvent("runtime.lane.created", command, ctx.eventLog, ctx.auditLog);
...
export function handleSessionAttach(
  ...
  if (!correlationId) {
    return {
      ...
      error: {
        code: "MISSING_CORRELATION_ID",
        message: "correlation_id is required for runtime.session.attach",
        retryable: false,
      },
    };
  }
  ...
  ctx.lifecycleProgress.get(correlationId)?.add("runtime.session.attach.started");
  publishLifecycleEvent("runtime.session.attach.started", command, ctx.eventLog, ctx.auditLog);

  if (forceError) {
    ctx.lifecycleProgress.get(correlationId)?.add("runtime.session.attach.failed");
    publishLifecycleEvent("runtime.session.attach.failed", command, ctx.eventLog, ctx.auditLog);
  ...
  ctx.lifecycleProgress.get(correlationId)?.add("runtime.session.attached");
  publishLifecycleEvent("runtime.session.attached", command, ctx.eventLog, ctx.auditLog);
  if (isRestore) {
...
export function handleTerminalSpawn(
  ...
  if (!correlationId) {
    return {
      ...
      error: {
        code: "MISSING_CORRELATION_ID",
        message: "correlation_id is required for runtime.terminal.spawn",
        retryable: false,
      },
    };
  }
  ...
  ctx.lifecycleProgress.get(correlationId)?.add("runtime.terminal.spawn.started");
  publishLifecycleEvent("runtime.terminal.spawn.started", command, ctx.eventLog, ctx.auditLog);

  if (forceError) {
    ctx.lifecycleProgress.get(correlationId)?.add("runtime.terminal.spawn.failed");
    publishLifecycleEvent("runtime.terminal.spawn.failed", command, ctx.eventLog, ctx.auditLog);
    return {
      id: `res-${Date.now()}`,
      type: "response",
      ts: new Date().toISOString(),
  ...
  publishLifecycleEvent("terminal.state.changed", command, ctx.eventLog, ctx.auditLog);
  ctx.lifecycleProgress.get(correlationId)?.add("runtime.terminal.spawned");
  publishLifecycleEvent("runtime.terminal.spawned", command, ctx.eventLog, ctx.auditLog);
