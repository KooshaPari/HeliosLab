...
export type RuntimeTerminalContext = {
	...
	getRuntimeState(): {
		lane: string;
		session: string;
		terminal: "active" | "throttled" | "idle";
	};
};
...
function appendTerminalOutput(
	...
	const lane_id =
		laneId ?? context.terminalRegistry.get(terminalId)?.lane_id ?? "";
	const session_id =
		sessionId ?? context.terminalRegistry.get(terminalId)?.session_id ?? "";
	...
	const outputEvt: LocalBusEnvelope = {
		...
		payload: {
			...
			runtime_state: context.getRuntimeState(),
		},
	};
...
export async function handleTerminalCommand(
	...
	if (command.method === "terminal.spawn") {
		...
		context.terminalRegistry.spawn({
			...
			title:
				typeof payload.title === "string" ? String(payload.title) : "Terminal",
		});
