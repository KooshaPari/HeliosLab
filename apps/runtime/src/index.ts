...
export function createRuntime(options: RuntimeOptions = {}) {
	...
	async function request(command: LocalBusEnvelope): Promise<LocalBusEnvelope> {
		...
		if (command.method === "terminal.resize" && response.status === "ok") {
			...
			const terminalId =
				command.terminal_id ??
				(command.payload as Record<string, unknown>)?.terminal_id;
	...
	async function fetch(requestInput: Request): Promise<Response> {
		...
		if (
			...
			if (lane.workspace_id !== workspaceId) {
				return Response.json(
					{ error: "does not belong to workspace" },
					{ status: 409 },
				);
			}
		...
		if (
			...
			return Response.json(
				{
					status:
						harnessStatus.status === "available" ? "available" : "unavailable",
	...
	function exportAuditBundle(filter?: {
		correlation_id?: string;
	}): RuntimeAuditBundle {
		const records = filter?.correlation_id
			? auditRecords.filter(
					(record) => record.correlation_id === filter.correlation_id,
				)
			: [...auditRecords];
		return {
