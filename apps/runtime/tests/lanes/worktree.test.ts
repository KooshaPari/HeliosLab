...
function createTempGitRepo(): string {
	...
	const modernInit = Bun.spawnSync(["git", "init", "--initial-branch=main"], {
		cwd: dir,
	});
	...
	const initialCommit = Bun.spawnSync(["git", "commit", "-m", "initial"], {
		cwd: dir,
	});
	...
	if (needsBranchRename) {
		const renameBranch = Bun.spawnSync(["git", "branch", "-M", "main"], {
			cwd: dir,
		});
