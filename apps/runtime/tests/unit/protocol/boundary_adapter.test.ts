...
describe("boundary adapter mapping", () => {
  test("maps local, tool, and a2a methods to explicit boundary adapters", () => {
    expect(getBoundaryDispatchDecision("runtime.terminal.spawn")).toEqual({
      boundary: "local_control",
      adapter: "local_bus",
    });
    expect(getBoundaryDispatchDecision("runtime.share.upterm.start")).toEqual({
      boundary: "tool_interop",
      adapter: "tool_bridge",
    });
    expect(getBoundaryDispatchDecision("runtime.agent.run")).toEqual({
  ...
  test("dispatches local methods through local adapter", async () => {
    const dispatcher = createBoundaryDispatcher({
      dispatchLocal: async () => ({
        ...
        type: "response",
        ts: "2026-02-27T00:00:00.000Z",
        method: "runtime.terminal.spawn",
        status: "ok",
        result: { ok: true },
      }),
    });

    const response = await dispatcher(command("runtime.terminal.spawn"));
  ...
  test("returns normalized fail-closed response when tool adapter is unavailable", async () => {
    ...
    const response = await dispatcher(command("runtime.boundary.tool.dispatch"));
    expect(response.type).toBe("response");
  ...
  test("routes a2a methods to provided adapter", async () => {
    const dispatcher = createBoundaryDispatcher({
      ...
      dispatchA2A: async () => ({
        ...
        type: "response",
        ts: "2026-02-27T00:00:00.000Z",
        method: "runtime.agent.run",
        status: "ok",
        result: { delegated: true },
      }),
    });

    const response = await dispatcher(command("runtime.agent.run"));
    expect(response.type).toBe("response");
    expect(response.status).toBe("ok");
