  ...
  test("rejects timestamps without RFC3339 timezone", () => {
    expect(() =>
      validateEnvelope({
        ...
        topic: "runtime.workspace.opened",
        payload: {},
      })
...
describe("protocol sequencing and audit", () => {
  test("stamps deterministic sequence for lifecycle events", async () => {
    ...
    expect(events[0]?.topic).toBe("runtime.session.attach.started");
    expect(events[1]?.topic).toBe("runtime.session.attached");
  ...
  test("rejects out-of-order lifecycle topic events", async () => {
    ...
    await expect(
      bus.publish({
        ...
        // biome-ignore lint/style/useNamingConvention: Protocol fixtures use protocol envelope snake_case keys.
        correlation_id: "corr-1",
        topic: "runtime.session.attached",
        payload: {},
      })
  ...
  test("records accepted and rejected publish attempts in audit sink", async () => {
    ...
    await bus.publish({
      ...
      topic: "runtime.lane.create.started",
      payload: {},
    });
  ...
  test("clears lifecycle progress after lane attach/cleanup success topics", async () => {
    ...
    await expect(
      bus.publish({
        ...
        topic: "runtime.lane.attach.started",
        payload: {},
      })
    ...
    await expect(
      bus.publish({
        ...
        topic: "runtime.lane.cleanup.started",
        payload: {},
      })
    ).resolves.toBeUndefined();
  ...
  test("keeps session detached when runtime.session.attach fails", async () => {
    const bus = new InMemoryLocalBus();
    ...
    expect(events).toHaveLength(2);
    expect(events[0]?.topic).toBe("runtime.session.attach.started");
    expect(events[1]?.topic).toBe("runtime.session.attach.failed");
  });
});
