...
describe("RendererPreferencesManager", () => {
	...
	it("should handle a preferences directory creation failure", () => {
		...
		try {
			const blockedManager = new RendererPreferencesManager(
				resolve(blockingPath, "prefs.json"),
			);

			expect(() =>
				blockedManager.save({ activeRenderer: "rio" }),
			).not.toThrow();
			...
			expect(errorLog.mock.calls[1]?.[0]).toBe(
				"Failed to save renderer preferences",
			);
