...
export const loadPluginSlates = async (): Promise<void> => {
	try {
		...
		const { initializeAllPluginRenderers } = await import(
			"./slates/pluginSlateRegistry"
		);
		if (slates && slates.length > 0) {
			const pluginNames = [
				...new Set(slates.map((s: PluginSlateInfo) => s.pluginName)),
			];
 ...
 */
export const findPluginSlateForFile = (
	filePath: string,
): PluginSlateInfo | null => {
	const pluginSlates = state.pluginSlates;
...
export const findPluginSlateForFolder = async (
	...
	try {
		const result = await electrobun.rpc?.request.pluginFindSlateForFolder({
			folderPath,
		});
		return result || null;
	} catch (e) {
