...
export const DiffEditor = ({
	...
	const [_hasSelection, setHasSelection] = createSignal(false);
      ...
      `;
		...
		editor.onDidUpdateDiff(() => {
			...
			if (lineChanges && lineChanges.length > 0) {
				...
				for (const change of lineChanges) {
					...
					if (startLine > 0) {
						stagingDecorations.push({
							...
							options: {
								glyphMarginClassName: isStaged
									? "staging-glyph-unstage"
									: "staging-glyph-stage",
								glyphMarginHoverMessage: {
									value: isStaged
										? "Click to unstage this change"
				...
				console.log(
					"Adding",
					stagingDecorations.length,
					"staging decorations in onDidUpdateDiff",
				);
          ...
          `;
				...
				const decorationIds = modifiedEditor.deltaDecorations(
					[],
					stagingDecorations,
				);
						...
						for (const change of lineChanges) {
							...
							if (startLine <= lineNumber && lineNumber <= endLine) {
								...
								if (!currentFilePath && editor) {
									const _lineChanges = editor.getLineChanges();
	...
	const prevDiff = () => {
		...
		currentDiffIndex =
			currentDiffIndex > 0 ? currentDiffIndex - 1 : lineChanges.length - 1;
		const change = lineChanges[currentDiffIndex];
	...
	const updateStagingDecorations = () => {
		...
		if (lineChanges && lineChanges.length > 0) {
			console.log(
				"FORCING staging decorations for",
				lineChanges.length,
				"changes",
			);
