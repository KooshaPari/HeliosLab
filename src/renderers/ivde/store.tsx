...
export const setPreviewNodeSlateConfig = (
...
export const openNewTabForNode = (
	...
	setState(
		produce((_state: AppState) => {
			...
			if (currentTabId && isPreview && existingTab?.isPreview) {
				if ("targetPaneId" in opts) {
					// if we're opening a node to a specific pane then just update the current preview Tab
			...
			const agentTabSettings =
				slateType === "agent"
					? { type: "agent" as const, title: slate?.name }
					: {};
...
export const splitPane = (
	...
	setState(
		produce((_state: AppState) => {
			...
			const _convertedContainer = reshapeObjectReference<LayoutContainerType>(
				paneToSplit,
				{
					id: getUniqueId(),
					type: "container",
...
export const isFileInProject = (absolutePath: string): boolean => {
	...
	for (const project of Object.values(projects)) {
		if (
			absolutePath.startsWith(`${project.path}/`) ||
