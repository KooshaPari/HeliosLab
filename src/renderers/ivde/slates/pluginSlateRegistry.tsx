...
import { registerSlateComponent } from "./PluginSlate";
...
export function registerSettingsComponent(
	...
	console.log(
		`[pluginSlateRegistry] Registered settings component: ${componentId}`,
	);
}
...
> = {
	"colab-webflow": () =>
		import("../../../../webflow-plugin/src/renderer/index"),
};
 ...
 */
export async function initializePluginRenderer(
	pluginName: string,
): Promise<void> {
	...
	try {
		...
		console.error(
			`[pluginSlateRegistry] Failed to load renderer for plugin ${pluginName}:`,
			e,
		);
	}
}
 ...
 */
export async function initializeAllPluginRenderers(
	pluginNames: string[],
): Promise<void> {
	const uniquePlugins = [...new Set(pluginNames)];
	await Promise.all(
		uniquePlugins.map((name) => initializePluginRenderer(name)),
	);
...
export function initializeSlateRegistry(): void {
	console.log(
		"[pluginSlateRegistry] Registry initialized (plugins will be loaded dynamically)",
	);
}
