...
export const PluginSlate = (props: PluginSlateProps) => {
			...
			if (renderData.html !== undefined) {
				...
				(window as any).colabSlate = {
					...
					querySelector: (selector: string) =>
						slateRoot?.querySelector(selector),
					querySelectorAll: (selector: string) =>
						slateRoot?.querySelectorAll(selector),
					root: slateRoot,
				};
	...
	onMount(async () => {
		...
		if (htmlMode) {
			console.log(
				`[PluginSlate] Using HTML-based rendering for slate: ${props.slateInfo.id}`,
			);
			// Listen for slateRender events
			window.addEventListener(
				"slateRender",
				handleSlateRender as EventListener,
			);
		}
		...
		try {
			...
			if (
				htmlMode &&
				result?.initialRenders &&
				result.initialRenders.length > 0
			) {
