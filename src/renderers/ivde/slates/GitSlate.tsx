...
export const GitSlate = ({ node }: { node?: CachedFileType }) => {
	...
	const { getFileDiff } = createGitContentReader(repoRootPath, {
		gitShow: electrobun.rpc?.request.gitShow?.bind(electrobun.rpc?.request),
		exists: electrobun.rpc?.request.exists?.bind(electrobun.rpc?.request),
		readFile: electrobun.rpc?.request.readFile?.bind(electrobun.rpc?.request),
	});
