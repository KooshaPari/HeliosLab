...
import { openNewTabForNode, state } from "../store";
...
const _parseStatusLine = (line: string) => {
...
export const GitSlate = ({ node }: { node?: CachedFileType }) => {
	...
	const getFileContents = async (
		...
		if (commitRef === "WORKING") {
			...
			const exists = await electrobun.rpc?.request.exists({
				path: absolutePath,
			});
			if (!exists) {
				return "";
	...
	createEffect(async () => {
		...
		if (relPath) {
			try {
				...
				if (
					uiState.originalText !== newOriginal ||
					uiState.modifiedText !== newModified
				) {
					setUiState({
	...
	const validateRemoteName = (
		...
		if (invalidChars.test(name)) {
			return {
				isValid: false,
				error: "Remote name cannot contain spaces or special characters",
			};
		}
	...
	const validateRemoteUrl = (
		...
		if (!isValidUrl) {
			return {
				isValid: false,
				error:
					"Invalid URL format (must start with https://, git@, ssh://, or file://)",
			};
		}
	...
	const [expandedRemotes, setExpandedRemotes] = createSignal<Set<string>>(
		new Set(),
	);
	...
	createEffect(() => {
		console.log(
			"Dialog state changed - dialogOpen:",
			dialogOpen(),
			"config:",
			dialogConfig(),
		);
		...
		try {
			...
			setTimeout(() => {
				if (descriptionRef) {
					...
					console.log(
						"Fallback: Textarea DOM value is now:",
	...
	const getLogAndStatus = async (resetPagination = true) => {
		...
		const [gitLog, gitStatus, shortStat, gitStashes, gitRemotes, gitBranches] =
			await Promise.all([
		...
		gitStatus?.files.forEach((file: any) => {
			if (file.path) {
				...
				if (
					...
					console.log(
						`  -> Added to staged: ${file.path} (${file.index.trim()})`,
					);
				}
				if (
					file.working_dir &&
					...
					console.log(
						`  -> Added to unstaged: ${file.path} (${file.working_dir.trim()})`,
					);
				}
			}
		});
		...
		const branches: BranchInfo = {
			...
			all: gitBranches?.all || [],
			remote: (gitBranches?.all || []).filter((b: string) =>
				b.includes("remotes/"),
			),
		};
	...
	const loadMoreCommits = async () => {
		...
		try {
			...
			if (gitLog?.all) {
				...
				const newCommits = gitLog.all.map((commit: any, _index: number) => {
	...
	const unstageFile = async (filePath: string) => {
		...
		if (selectedFile().relPath === filePath) {
			...
			if (currentSelection.isFromStaged) {
				...
				const hasUnstagedChanges = Object.keys(
					uiState.changes.unstaged || {},
				).includes(filePath);
	...
	const stageLines = async (
		...
		try {
			...
			if (
				...
				const _result = await electrobun.rpc?.request.gitStageMonacoChange({
					repoRoot: repoRootPath,
					filePath: filePath,
				...
				const _result = await electrobun.rpc?.request.gitStageSpecificLines({
					repoRoot: repoRootPath,
					filePath: filePath,
					startLine: startLine,
					endLine: endLine,
			...
			throw new Error(
				`Failed to stage lines ${startLine}-${endLine}: ${error}`,
			);
		}
	};
	...
	const unstageLines = async (
		...
		try {
			...
			if (
				...
				const _result = await electrobun.rpc?.request.gitUnstageMonacoChange({
					repoRoot: repoRootPath,
					filePath: filePath,
			...
			throw new Error(
				`Failed to unstage lines ${startLine}-${endLine}: ${error}`,
			);
		}
	};
	...
	const _popStash = async (stashName: string) => {
		try {
	...
	const discardFileChanges = (filePath: string) => {
		...
		setDialogConfig({
			...
			message,
			confirmText:
				change.changeType === "?" || change.changeType === "A"
					? "Delete"
					: "Discard",
			type: "danger",
			onConfirm: async () => {
		...
		setDialogConfig({
			...
			onConfirm: async () => {
				...
				try {
					...
					for (const [filePath, change] of Object.entries(
						uiState.changes.unstaged,
					)) {
						if (change.changeType === "?" || change.changeType === "A") {
	...
	const _onClickRestore = async (commit: CommitType) => {
			...
			>
				...
				>
					...
					>
						...
						>
							<span>
								Changes ({Object.keys(uiState.changes.unstaged || {}).length})
							</span>
							<Show
						...
						<Show when={Object.keys(uiState.changes.unstaged || {}).length > 0}>
							...
							>
								<FileList
									...
									commitHash="HEAD"
									onClick={(change, commitHash) =>
										onClickChange(change, commitHash, false)
									}
						...
						>
							...
							>
											...
											<input
												...
												onChange={(e) => {
													...
													if (checked) {
														...
														const lastCommit = uiState.log?.find(
															(c) => !c.isRemoteOnly,
														);
														if (lastCommit) {
								...
								<FileList
									...
									commitHash="HEAD"
									onClick={(change, commitHash) =>
										onClickChange(change, commitHash, true)
									}
									selectedFile={selectedFile}
									showStageButtons={true}
									onStage={stageFile}
										...
										<Show when={uiState.syncStatus.ahead > 0}>
											<span
												style={{
													color: "#4ade80",
													"margin-left":
														uiState.syncStatus.behind > 0 ? "4px" : "0",
												}}
											>
								...
								<div style={{ display: "flex", gap: "4px" }}>
									...
									<button
										...
										style={{
											...
											gap: "4px",
											opacity:
												uiState.syncStatus?.ahead > 0 || isAltPressed()
													? "1"
													: "0.5",
											"white-space": "nowrap",
										}}
										disabled={
											!isAltPressed() && uiState.syncStatus?.ahead === 0
										}
										onClick={async () => {
							...
							<For each={uiState.log}>
								{(commit, index) => {
									...
									const [isExpanded, setIsExpanded] =
										createSignal(initialExpanded);
									const [isCommitHovered, setIsCommitHovered] =
										createSignal(false);
											...
											>
												...
												<div
													style={{
														...
														height: "8px",
														"border-radius": "50%",
														background: commit.isRemoteOnly
															? "#858585"
															: "#0078d4",
														"margin-right": "8px",
														"flex-shrink": "0",
														...
														{([filepath, filechange]) => {
															...
															return (
																<div
																	...
																	onClick={() =>
																		onClickChange(filechange, commit.hash)
																	}
																>
																	<ChangeTypeSpan
								...
								>
									<button
										...
										style={{
											...
											border:
												"1px solid " +
												(pagination.isLoading ? "#555" : "#1f6feb"),
					...
					>
						...
						>
							...
							>
								...
								>
									BRANCHES (
									{uiState.branches?.all?.filter((b) => !b.includes("remotes/"))
										.length || 0}
									)
								</button>
								<button
						...
						>
							...
							<Show when={uiState.activeSection === "branches"}>
								<div style={{ padding: "8px 0" }}>
									...
									>
										<For
											each={uiState.branches.all.filter(
												(b) => !b.includes("remotes/"),
											)}
										>
											{(branch) => {
												...
												return (
													...
													>
														...
														>
															...
															{isCurrent &&
																!uiState.branches.trackingBranch && (
																	<span
																		style={{
																			"font-size": "10px",
																			color: "#f59e0b",
																...
																<button
																	...
																	onClick={async (e) => {
																		...
																		try {
																			...
																			console.log(
																				"Branch published, refreshing status...",
																			);
																			// Small delay to ensure git operation is fully committed
																			...
																			await getLogAndStatus();
																			console.log(
																				"Status refreshed after publish",
																			);
																		} catch (error) {
																			console.error(
																			...
																			alert(
																				`Failed to publish branch: ${error}`,
																			);
																		}
																	}}
																	onMouseEnter={(e) => {
																...
																<button
																	...
																	onClick={async (e) => {
																		...
																		try {
																			...
																			console.log(
																				"Branch checked out, refreshing status...",
																			);
																			// Small delay to ensure git operation is fully committed
																			await new Promise((resolve) =>
																				setTimeout(resolve, 100),
																			...
																			await getLogAndStatus();
																			console.log(
																				"Status refreshed after checkout",
																			);
																		} catch (error) {
																			console.error(
											...
											fallback={
												...
												>
													<input
														...
														onInput={(e) =>
															setBranchInputValue(e.currentTarget.value)
														}
														style={{
															background: "#1e1e1e",
														...
														onKeyDown={(e) => {
															if (e.key === "Enter") {
																e.preventDefault();
																const validation = validateBranchName(
																	branchInputValue(),
																);
																if (validation.isValid) {
																	handleCreateBranch();
														...
														<button
															...
															style={{
																background: validateBranchName(
																	...
																	: "#333",
																border: "1px solid #094771",
																color: validateBranchName(branchInputValue())
																	.isValid
																	? "#fff"
																	: "#999",
																"font-size": "10px",
																...
																padding: "4px 8px",
																"border-radius": "3px",
																cursor: validateBranchName(branchInputValue())
																	.isValid
																	? "pointer"
																	: "not-allowed",
																"font-family":
										...
										>
											<div style={{ "margin-bottom": "8px" }}>
												<input
													...
													onInput={(e) =>
														setRemoteNameValue(e.currentTarget.value)
													}
													style={{
														background: "#252526",
														border: "1px solid #094771",
													...
													onKeyDown={(e) => {
														if (e.key === "Enter") {
															e.preventDefault();
															const nameValid = validateRemoteName(
																remoteNameValue(),
															).isValid;
															const urlValid = validateRemoteUrl(
																remoteUrlValue(),
															).isValid;
												...
												<input
													...
													onInput={(e) =>
														setRemoteUrlValue(e.currentTarget.value)
													}
													style={{
														background: "#252526",
														border: "1px solid #094771",
												...
												return (
													<div style={{ "border-bottom": "1px solid #2d2d2d" }}>
														...
														<Show when={isExpanded()}>
															...
															>
																<For
																	each={uiState.branches.remote.filter(
																		(b: string) =>
																...
																>
																	{(branch) => {
																		...
																		const isCurrent =
																			branchName === uiState.branches.current;
																		return (
																			<div
																				style={{
																					padding: "4px 12px",
																				...
																				<Show when={!isCurrent}>
																					{(() => {
																						const hasLocalBranch =
																							uiState.branches.all.includes(
																								branchName,
																							);
																						const buttonText = hasLocalBranch
																							? "switch"
																							: "track";
																						const borderColor = hasLocalBranch
																							? "#555"
																							: "#4fc3f7";
										...
										>
											<div style={{ "margin-bottom": "8px" }}>
												...
												<input
													...
													value={stashMessage()}
													onInput={(e) =>
														setStashMessage(e.currentTarget.value)
													}
													style={{
														width: "100%",
														background: "#3c3c3c",
											...
											<div style={{ "margin-bottom": "12px" }}>
												...
												>
													<input
														...
														onChange={(e) =>
															setIncludeUntracked(e.currentTarget.checked)
														}
														style={{
															"margin-right": "6px",
															"accent-color": "#0078d4",
