...
export const TerminalSlate = ({ tabId }: { tabId: string }) => {
	...
	const updateCurrentDir = async () => {
		...
		try {
			const cwd = await electrobun.rpc?.request.getTerminalCwd({
				terminalId: id,
			});
	...
	const initializeTerminal = async () => {
		...
		try {
			...
			const webLinksAddon = new WebLinksAddon(
				...
				{
					hover: (event: MouseEvent, uri: string, _range) => {
	...
	onMount(() => {
		...
		const handleTerminalExit = (
			event: CustomEvent<{ terminalId: string; exitCode: number }>,
		) => {
			const data = event.detail;
			if (data.terminalId === terminalId() && terminal) {
				terminal.write(
		...
		window.addEventListener(
			"terminalExit",
			handleTerminalExit as EventListener,
		);
