...
import { SettingsPaneField, SettingsPaneFormSection } from "./forms";
...
const CustomSettingsLoader = (props: {
	...
	const sendMessage = async (message: unknown) => {
		console.log(
			"[PluginSettings] sendMessage called:",
			props.pluginName,
			message,
		);
		try {
	...
	onMount(() => {
		const pollMessages = async () => {
			try {
				const messages =
					await electrobun.rpc?.request.pluginGetPendingSettingsMessages({
						pluginName: props.pluginName,
					});
	...
	// State helpers
	const getStateValue = async <T = unknown>(
		key: string,
	): Promise<T | undefined> => {
	...
	const setStateValue = async <T = unknown>(
		key: string,
		value: T,
	): Promise<void> => {
...
export const PluginSettings = (): JSXElement => {
	...
	const loadSettings = async () => {
		...
		try {
			const [schemaResult, valuesResult, entitlementsResult, validationResult] =
				await Promise.all([
					...
					electrobun.rpc?.request.pluginGetSettingValidationStatuses({
						pluginName,
					}),
				]);
	...
	const pollValidationStatus = async (key: string) => {
		...
		const poll = async () => {
			...
			try {
				const statuses =
					await electrobun.rpc?.request.pluginGetSettingValidationStatuses({
						pluginName,
					});
				if (statuses?.[key]) {
					...
					if (
						statuses[key].state === "valid" ||
						statuses[key].state === "invalid"
					) {
						return;
					}
				}
	...
	const handleValueChange = async (
		...
		// Persist to backend
		try {
			await electrobun.rpc?.request.pluginSetSettingValue({
				pluginName,
				key,
				value,
			});
										...
										<Switch>
											...
											<Match when={field.type === "string"}>
												<input
													...
													onInput={(e) =>
														handleValueChange(field.key, e.currentTarget.value)
													}
												...
												<div style="position: relative;">
													...
													>
														...
														>
															...
															>
																<span style="color: #51cf66; font-size: 14px;">
																	✓
																</span>
															</Show>
															<Show
											...
											<Match when={field.type === "select"}>
												<select
													...
													onChange={(e) => {
														...
														handleValueChange(
															field.key,
															Number.isNaN(numVal) ? val : numVal,
														);
													}}
								...
								<div style="padding: 12px 16px;">
									<div style="background: #2a2a2a; border: 1px solid #444; border-radius: 6px; padding: 12px; margin-bottom: 12px;">
										<div style="display: flex; align-items: flex-start; gap: 8px; color: #f0ad4e; font-size: 11px;">
											...
											<div>
												<strong>Trust Notice:</strong> These are capabilities
												the plugin author declares it needs. They are{" "}
												<em>not enforced</em> by Colab. Only install plugins
