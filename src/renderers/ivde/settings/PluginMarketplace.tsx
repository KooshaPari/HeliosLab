...
import { openNewTabForNode, setState } from "../store";
...
export const PluginMarketplace = (): JSXElement => {
	...
	const [installedPlugins, setInstalledPlugins] = createSignal<
		InstalledPlugin[]
	>([]);
	const [pluginEntitlements, setPluginEntitlements] = createSignal<
		Record<string, EntitlementSummary[]>
	>({});
	const [expandedEntitlements, setExpandedEntitlements] = createSignal<
		string | null
	>(null);
	...
	const [activeTab, setActiveTab] = createSignal<"browse" | "installed">(
		"browse",
	);
			...
			for (const plugin of plugins || []) {
				try {
					const pluginEntitlements =
						await electrobun.rpc?.request.pluginGetEntitlements({
	...
	const handleUninstall = async (packageName: string) => {
		setInstalling(packageName);
		try {
			const result = await electrobun.rpc?.request.pluginUninstall({
				packageName,
			});
			if (result?.ok) {
				await loadInstalledPlugins();
	...
	const handleInstallFromFolder = async () => {
		try {
			...
			setError(
				err instanceof Error ? err.message : "Failed to install from folder",
			);
		} finally {
			setInstalling(null);
		}
										...
										<Show when={isInstalled(ext.name)}>
											<button type="button"
												...
												style={{
													...
													"border-radius": "3px",
													cursor:
														installing() === ext.name ? "wait" : "pointer",
													"font-size": "11px",
								...
								>
									...
									<span
										style={{
											...
											"border-radius": "3px",
											background:
												plugin.state === "active" ? "#1a3d1a" : "#3d3d1a",
