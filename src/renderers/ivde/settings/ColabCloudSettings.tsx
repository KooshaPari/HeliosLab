...
export const ColabCloudSettings = (): JSXElement => {
	...
	const isConnected = () => {
		return (
			state.appSettings.colabCloud?.accessToken &&
	...
	const handleRestore = async () => {
		...
		if (result.success) {
			...
			showSyncMessage({
				type: "error",
				text: result.error || "Restore failed. Wrong passphrase?",
			});
		}
	};
					...
					<Show when={isConnected()}>
							...
							<div style="background: #2b2b2b; padding: 12px; border-radius: 4px;">
								<div style="display: flex; flex-direction: column; gap: 4px;">
									<span style="font-size: 12px; font-weight: 500; color: #d9d9d9;">
										{state.appSettings.colabCloud?.name ||
											state.appSettings.colabCloud?.email}
									</span>
						...
						<SettingsPaneField label="Connected">
							<div style="font-size: 11px; color: #999;">
								Connected on{" "}
								{formatDateShort(state.appSettings.colabCloud?.connectedAt)}
							</div>
						</SettingsPaneField>
					</Show>
				</SettingsPaneFormSection>
					...
					<SettingsPaneFormSection label="Settings Backup">
						...
						<Show when={!hasPassphrase() && !isSettingPassphrase()}>
							<SettingsPaneField label="">
								<div
									role="button"
									tabIndex={0}
									style="background: rgba(255, 193, 7, 0.1); border: 1px solid rgba(255, 193, 7, 0.3); padding: 16px; border-radius: 4px; text-align: center;"
								>
									...
									<div style="font-size: 11px; color: #999; margin-bottom: 12px;">
										Your settings are encrypted locally before upload. We never
										see your data.
									</div>
									<button
										type="button"
						...
						<Show when={isSettingPassphrase()}>
							<SettingsPaneField label="Create Encryption Passphrase">
								<div style="font-size: 11px; color: #999; margin-bottom: 12px;">
									This passphrase encrypts your settings. You'll need it to
							...
							<SettingsPaneField label="">
								...
								>
									<button
										...
										onKeyDown={(e) => {
											if (e.key === "Enter" || e.key === " ") handleBackup;
										}}
										disabled={isSyncing()}
