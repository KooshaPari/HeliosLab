...
import { setState, state, updateSyncedAppSettings } from "../store";
import {
	SettingsPageShell,
	SettingsPaneField,
...
export const LlamaSettings = (): JSXElement => {
	...
	const [statusMessage, setStatusMessage] =
		createSignal<string>("Loading models...");
	const [installingModel, setInstallingModel] = createSignal<string | null>(
		null,
	);
	const [installProgress, setInstallProgress] = createSignal<string>("");
	const [_downloadId, setDownloadId] = createSignal<string | null>(null);
	const [uninstallingModel, setUninstallingModel] = createSignal<string | null>(
		null,
	);
	...
	const pollDownloadStatus = async (downloadId: string) => {
		const poll = async () => {
			try {
				const result = await electrobun.rpc?.request.llamaDownloadStatus({
					downloadId,
				});
				...
				<SettingsPaneFormSection label="Status">
					<SettingsPaneField label="Models">
						...
						<div style="margin-top: 8px; font-size: 11px; color: #999;">
							Models are managed locally using llama.cpp. No external service
							required.
						</div>
					</SettingsPaneField>
				</SettingsPaneFormSection>
						...
						>
							{availableModels().length > 0 ? (
								availableModels().map((model) => (
									...
									>
										{model.name} (
										{model.source === "llama" ? "llama.cpp" : "legacy"},{" "}
						...
						<div style="display: flex; flex-direction: column; gap: 8px;">
							<div style="display: flex; align-items: center; gap: 12px;">
								<input
									...
									onInput={(e) =>
										setCurrentTemperature(parseFloat(e.currentTarget.value))
									}
							...
							<div style="font-size: 11px; color: #999; margin-top: 4px;">
								0.0-0.3: Focused (good for code) • 0.4-0.7: Balanced • 0.8-1.0:
								Creative • 1.0+: Very random
							</div>
						</div>
					</SettingsPaneField>
				</SettingsPaneFormSection>
											...
											<div style="display: flex; justify-content: space-between; align-items: center; padding: 8px; background: #2b2b2b; border-radius: 4px;">
												<div style="display: flex; flex-direction: column;">
													...
													<span style="font-size: 10px; color: #999; margin-top: 2px;">
														{formatFileSize(model.size)} • Source:{" "}
														{model.source}
													</span>
												</div>
												<div>
													{isUninstalling ? (
														<div style="display: flex; align-items: center; gap: 4px;">
															...
															<span style="font-size: 10px; color: #ff6b6b;">
																Removing...
															</span>
														</div>
													) : model.source === "llama" ? (
														<button
															type="button"
															onClick={() =>
																uninstallModel(model.path, model.name)
															}
						...
						<SettingsPaneField label="">
							<div style="padding: 12px; background: #1a1a1a; border: 1px solid #333; border-radius: 4px; margin-bottom: 8px;">
								<div style="font-size: 12px; color: #ffa500; display: flex; align-items: center; gap: 8px;">
									...
									<span>
										{installProgress() || `Installing ${installingModel()}...`}
									</span>
								</div>
							</div>
						</SettingsPaneField>
						...
						<div style="display: flex; flex-direction: column; gap: 6px;">
							<For each={popularModels}>
								{(model) => {
									...
									const expectedModelName = expectedFileName.replace(
										".gguf",
										"",
									);
									const isInstalled = () =>
