...
export const GitHubSettings = (): JSXElement => {
	...
	onMount(async () => {
		...
		try {
			...
			const credentials =
				await electrobun.rpc?.request.checkGitHubCredentials();
			if (credentials) {
			...
			<SettingsPaneFormSection label="Git">
				...
				<SettingsPaneField label="Author Email">
					...
					<div
						role="button"
						tabIndex={0}
						style="font-size: 10px; color: #777; margin-top: 4px;"
					>
						Used for commit author attribution
					</div>
				</SettingsPaneField>
				...
				<SettingsPaneField label="">
					<button
						...
						onKeyDown={(e) => {
							if (e.key === "Enter" || e.key === " ") saveIdentity;
						}}
						style={{
						...
						<>
							...
							<SettingsPaneField label="">
								<div
									role="button"
									tabIndex={0}
									style="background: #1a1a1a; border: 1px solid #333; padding: 12px; border-radius: 4px; margin-bottom: 8px;"
								>
									...
									<a
										...
										onKeyDown={(e) => {
											if (e.key === "Enter" || e.key === " ") openTokenPage;
										}}
							...
							<SettingsPaneField label="">
								<button
									...
									onKeyDown={(e) => {
										if (e.key === "Enter" || e.key === " ") connectGitHub;
									}}
							...
							<Show when={!hasKeychainHelper()}>
								<SettingsPaneField label="">
									<div style="background: #3d2020; border: 1px solid #5a3030; padding: 12px; border-radius: 4px; font-size: 11px; color: #ff9999;">
										macOS Keychain helper not available. Install Xcode Command
										Line Tools to enable secure credential storage.
									</div>
								</SettingsPaneField>
							</Show>
						</>
								...
								<div style="display: flex; align-items: center; gap: 8px;">
									<div
										style={{
											...
											height: "8px",
											"border-radius": "50%",
											background: keychainCredentials().hasCredentials
												? "#51cf66"
												: "#ffa500",
										}}
									></div>
