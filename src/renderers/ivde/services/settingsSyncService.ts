...
export async function gatherSyncableSettings(): Promise<SyncedSettings> {
	...
	try {
		const pluginsResult = await (
			electrobun.rpc as any
		)?.request.pluginGetInstalled?.();
			...
			for (const plugin of pluginsResult) {
				...
				try {
					const settingsResult = await (
						electrobun.rpc as any
					)?.request.pluginGetSettingsValues?.({ pluginName: plugin.name });
	...
	if (settings.plugins && settings.plugins.length > 0) {
		for (const plugin of settings.plugins) {
			try {
				...
				const installedPlugins = await (
					electrobun.rpc as any
				)?.request.pluginGetInstalled?.();
				const isInstalled = installedPlugins?.some(
					(p: any) => p.name === plugin.name,
				);
