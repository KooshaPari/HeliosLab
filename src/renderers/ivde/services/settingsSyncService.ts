...
type SettingsSyncRpcRequest = {
	...
	pluginSetSettingValue?: (args: {
		pluginName: string;
		key: string;
		value: unknown;
	}) => Promise<unknown>;
	pluginSetEnabled?: (args: {
		packageName: string;
		enabled: boolean;
	}) => Promise<unknown>;
};
	...
	try {
		...
		if (Array.isArray(pluginsResult)) {
			for (const plugin of pluginsResult) {
				...
				try {
					const settingsResult = await request?.pluginGetSettingsValues?.({
			...
			try {
				...
				if (plugin.settings) {
					for (const [key, value] of Object.entries(plugin.settings)) {
						await request?.pluginSetSettingValue?.({
							pluginName: plugin.name,
							key,
							value,
						});
				...
				// Set enabled state
				await request?.pluginSetEnabled?.({
					packageName: plugin.name,
					enabled: plugin.enabled,
				});
			} catch (error) {
...
export async function downloadSettings(
	...
	try {
		...
		try {
			settings = await decryptSettings<SyncedSettings>(
				encryptedPayload,
				passphrase,
			);
		} catch (_decryptError) {
