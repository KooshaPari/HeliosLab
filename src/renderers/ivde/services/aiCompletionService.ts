...
function cleanInlineCompletion(
	raw: string,
	_langHint: string | undefined, // e.g. "typescript" or file extension
	{
		maxLines = 3,
	...
	if (/^(.)\1{10,}$/.test(s)) {
		console.log(
			"🚫 Filtering out repetitive completion:",
			`${s.slice(0, 20)}...`,
		);
		return "";
	}
...
class AICompletionService {
	...
	cancelAllPendingRequests() {
		console.log(
			`🚫 Cancelling ${this.activeAbortControllers.size} pending AI requests`,
		);
	...
	async getInlineCompletions(
		...
		try {
			...
			const lastWord =
				currentLineContent.match(/[a-zA-Z_$][a-zA-Z0-9_$]*$/)?.[0] || "";
			...
			if (
				lastWord.length > 0 &&
				completion.toLowerCase().startsWith(lastWord.toLowerCase())
			) {
			...
			if (
				lastWord.length === 1 &&
				commonNonCodeWords.some(
					(word) => completion.toLowerCase().trim() === word.toLowerCase(),
				)
			) {
				return { items: [] };
			}
