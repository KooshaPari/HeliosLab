		...
		>
			...
			<div
				...
				onMouseEnter={(e) => {
					...
					const bunnyImg = e.currentTarget.querySelector(
						"[data-bunny-btn]",
					) as HTMLElement;
					if (tip) {
						tip.style.opacity = "1";
			...
			>
				...
				<div
					...
					onClick={(e) => {
						electrobun.rpc?.send.openBunnyWindow({
							screenX: e.screenX,
							screenY: e.screenY,
						});
					}}
			...
			<div
				...
				onClick={() => {
					setState("settingsPane", {
						type:
							state.settingsPane.type === "colab-cloud-settings"
								? ""
			...
			>
				...
				<span style="color: #4ade80; font-weight: 500; font-size: 12px;">
					Cloud
				</span>
			</div>
...
const CommandPalette = ({ setOpen }: { setOpen: (value: boolean) => void }) => {
		...
		const tabs = Object.values(getWindow(state)?.tabs || {}).reduce(
			(acc, tab) => {
				if (tab.type === "file") {
					...
					const projectName =
						project?.name || (project?.path ? basename(project.path) : "");
	...
	const pluginsClick = () => {
		setState("settingsPane", {
			type:
				state.settingsPane.type === "plugin-marketplace"
					? ""
					: "plugin-marketplace",
			data: {},
		});
		...
		if (query) {
			...
			Object.entries(results).forEach(([key, value]) => {
				...
				value.forEach((path) => {
					...
					const _projectFolderName = basename(project.path);
...
const _WorkspaceMenu = ({ children }: { children: JSX.Element }) => {
	return (
...
const Update = () => {
	...
	const buttonTitle = () => {
		...
		if (isReady()) {
			return version
				? `Click to restart and update to v${version}`
