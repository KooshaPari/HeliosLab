...
import { basename, relative } from "../utils/pathUtils";
import { _getNode, createModel } from "./FileWatcher";
...
export const Editor = ({ currentTabId }: { currentTabId: string }) => {
	...
	const setModel = async () => {
		...
		if (
			tabWithSelection &&
			"selection" in tabWithSelection &&
			tabWithSelection.selection
		) {
					...
					monaco.languages.registerInlineCompletionsProvider(lang, {
						provideInlineCompletions: async (
							...
							try {
								...
								const _lineStartOffset = model.getOffsetAt({
									lineNumber: position.lineNumber,
									column: 1,
								});
								...
								const aiResponse =
									await aiCompletionService.getInlineCompletions({
										prefix: codePrefix,
										suffix: codeSuffix,
										language: "typescript",
