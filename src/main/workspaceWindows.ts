...
export const clearFocusedWindow = (workspaceId: string, windowId: string) => {
	...
	if (
		focusedWindowInfo?.workspaceId === workspaceId &&
		focusedWindowInfo?.windowId === windowId
	) {
...
export const sendToFocusedWindow = (type: string, data: any) => {
	console.log(
		`sendToFocusedWindow: type=${type}, focusedWindowInfo=`,
		focusedWindowInfo,
	);
	if (focusedWindowInfo) {
		...
		if (activeWorkspaceWindows?.[windowId]) {
			console.log(
				`sendToFocusedWindow: sending to workspace=${workspaceId}, window=${windowId}`,
			);
	...
	console.log(
		`sendToFocusedWindow: no focused window, falling back to broadcast`,
	);
