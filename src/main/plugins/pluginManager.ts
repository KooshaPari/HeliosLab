...
class PluginManager {
	...
	private settingsChangeCallbacks: Map<string, Set<SettingsChangeCallback>> =
		new Map(); // plugin name -> callbacks
	private settingsValidationStatuses: Map<string, SettingValidationStatuses> =
		new Map(); // plugin name -> key -> validation status
	private settingsMessageCallbacks: Map<
		string,
		Set<(message: unknown) => void>
	> = new Map(); // plugin name -> message callbacks
	...
	private slateRenderCallbacks: Map<
		string,
		(message: SlateRenderMessage) => void
	> = new Map(); // instanceId -> render callback
	...
	private slateWindowMessageHandler:
		| ((windowId: string, message: unknown) => void)
		| null = null;
	...
	private getSettingsFilePath(pluginName: string): string {
		return join(
			COLAB_PLUGINS_PATH,
			`${pluginName.replace(/\//g, "__")}.settings.json`,
		);
	}
	...
	private getStateFilePath(pluginName: string): string {
		return join(
			COLAB_PLUGINS_PATH,
			`${pluginName.replace(/\//g, "__")}.state.json`,
		);
	}
	...
	async installFromLocalPath(localPath: string): Promise<InstalledPlugin> {
		console.info(
			`[PluginManager] Installing plugin from local path: ${localPath}`,
		);
		...
		const result = execSpawnSync(BUN_BINARY_PATH, ["add", fileSpec], {
		...
		const installedPackageJson = JSON.parse(
			readFileSync(installedPackageJsonPath, "utf-8"),
		);
		...
		if (installedPlugin.enabled) {
			try {
				...
				console.warn(
					`[PluginManager] Failed to auto-activate ${packageName}:`,
					error,
				);
	...
	async installFromNpm(
		...
		if (!existsSync(packageJsonPath)) {
			throw new Error(
				`Failed to install plugin: package.json not found at ${packageJsonPath}`,
			);
		}
	...
	async uninstallPlugin(packageName: string): Promise<void> {
		...
		// Run bun remove
		execSpawnSync(BUN_BINARY_PATH, ["remove", packageName], {
			cwd: COLAB_PLUGINS_PATH,
		});
	...
	async updatePlugin(packageName: string): Promise<InstalledPlugin> {
		...
		// Run bun update
		execSpawnSync(BUN_BINARY_PATH, ["update", packageName], {
			cwd: COLAB_PLUGINS_PATH,
		});
	...
	async activatePlugin(packageName: string): Promise<void> {
		...
		if (!plugin.enabled) {
			console.info(
				`[PluginManager] Plugin ${packageName} is disabled, skipping activation`,
			);
			return;
		}
		...
		try {
			const entryPath = join(
				plugin.installPath,
				plugin.manifest.main || "dist/index.js",
			);
			...
			if (pluginModule.activate) {
				...
				console.warn(
					`[PluginManager] Plugin ${packageName} has no activate function`,
				);
			}
			...
			console.info(
				`[PluginManager] Plugin ${packageName} activated successfully`,
			);
		} catch (error) {
			console.error(
				`[PluginManager] Failed to activate ${packageName}:`,
				error,
			);
			plugin.state = "error";
	...
	private entitlementsToPermissions(
		...
		if (
			...
			permissions.terminal = entitlements.terminal.write
				? "readwrite"
				: "readonly";
		}
		...
		return {
			...
			webview: {
				registerPreloadScript(script: string) {
					...
					console.info(
						`[PluginManager] Plugin ${pluginName} registered preload script`,
					);
					return {
						dispose: () => {
			...
			terminal: {
				async createTerminal(_options: { name?: string; cwd?: string }) {
				...
				async sendText(_terminalId: string, _text: string) {
					if (permissions.terminal !== "readwrite") {
						throw new Error(
							"Permission denied: terminal:readwrite access required",
						);
					}
					// TODO: implement
				},
				registerCommand(name: string, handler: TerminalCommandHandler) {
					...
					console.info(
						`[PluginManager] Plugin ${pluginName} registered terminal command: ${name}`,
					);
					return {
						dispose: () => {
			...
			shell: {
				async exec(
					...
					const manifest =
						self.refreshPluginManifest(pluginName) || plugin?.manifest;
					if (!manifest?.entitlements?.process?.spawn) {
						...
						throw new Error(
							"Permission denied: process.spawn entitlement required",
						);
					}
					...
					try {
						...
						console.info(
							`[Plugin:${pluginName}] shell.exec completed successfully`,
						);
						return {
						...
						console.warn(
							`[Plugin:${pluginName}] shell.exec failed:`,
							e.message,
						);
						return {
							stdout: e.stdout || "",
				...
				async openExternal(target: string) {
					...
					return new Promise((resolve, reject) => {
						exec(command, (error) => {
							if (error) {
								console.error(
									`[Plugin:${pluginName}] shell.openExternal failed:`,
									error,
								);
								reject(error);
							} else {
			...
			git: {
				async getStatus(_repoRoot: string) {
				...
				async getBranch(_repoRoot: string) {
					if (permissions.git === "none") {
						throw new Error("Permission denied: git access required");
					}
					// TODO: implement
			...
			configuration: {
				...
				async update(_key: string, _value: unknown) {
					// TODO: implement
				},
			},

			events: {
				onFileChange(_callback: (event: any) => void) {
					// TODO: implement
					return { dispose: () => {} };
				},
				onActiveEditorChange(_callback: (editor: any) => void) {
					// TODO: implement
					return { dispose: () => {} };
				},
			},
			...
			statusBar: {
				createItem(item: StatusBarItem) {
					...
					self.statusBarItems.set(fullId, {
						pluginName,
						item: { ...item, id: fullId },
					});
					console.info(
						`[PluginManager] Plugin ${pluginName} created status bar item: ${item.id}`,
					);
			...
			fileDecorations: {
				registerProvider(provider: FileDecorationProvider) {
					...
					console.info(
						`[PluginManager] Plugin ${pluginName} registered file decoration provider`,
					);
	...
	async deactivatePlugin(packageName: string): Promise<void> {
		...
		try {
			...
			console.warn(
				`[PluginManager] Error during deactivation of ${packageName}:`,
				error,
			);
		}
	...
	broadcastEvent(eventType: string, payload: unknown): void {
		for (const [_name, workerState] of this.activeWorkers) {
		...
		if (plugin) {
			...
			if (!enabled && this.activeWorkers.has(name)) {
				...
				try {
					...
					console.error(
						`[PluginManager] Failed to activate plugin ${name}:`,
						e,
					);
				}
			}
	...
	private refreshPluginManifest(name: string): PluginManifest | undefined {
		...
		try {
			...
			const basePath =
				plugin.isLocal && plugin.localPath
					? plugin.localPath
					: plugin.installPath;
			...
			console.error(
				`[PluginManager] Failed to refresh manifest for ${name}:`,
				e,
			);
		}
	...
	getAllPreloadScripts(): string {
		...
		console.info(
			`[PluginManager] Returning ${scripts.length} total preload scripts`,
		);
		return scripts.join("\n\n");
	}
	...
	async executeTerminalCommand(
		...
		try {
			...
			console.error(
				`[PluginManager] Terminal command "${commandName}" failed:`,
				error,
			);
		}
	...
	getTriggerCharacters(language: string): string[] {
		...
		for (const [, registered] of this.completionProviders) {
			if (
				registered.languages.includes(language) &&
				registered.provider.triggerCharacters
			) {
	...
	getStatusBarItems(): Array<
		StatusBarItem & { pluginName: string; hasSettings: boolean }
	> {
	 ...
	 */
	async getFileDecoration(
		filePath: string,
	): Promise<FileDecoration | undefined> {
		for (const [, registered] of this.decorationProviders) {
			try {
				const decoration =
					await registered.provider.provideDecoration(filePath);
				if (decoration) {
					return decoration;
				}
			} catch (error) {
				console.error(
					`[PluginManager] File decoration provider failed for ${filePath}:`,
					error,
				);
			}
		}
	...
	getContextMenuItems(
		...
		for (const [, registered] of this.contextMenuItems) {
			if (
				registered.item.context === contextType ||
				registered.item.context === "both"
			) {
				items.push({
	...
	async executeContextMenuItem(
		...
		try {
			...
			console.error(
				`[PluginManager] Context menu handler failed for ${itemId}:`,
				error,
			);
		}
	}
	 ...
	 */
	getPluginSettingValidationStatuses(
		pluginName: string,
	): SettingValidationStatuses {
	...
	sendSettingsMessage(pluginName: string, message: unknown): void {
		console.log(
			`[PluginManager] sendSettingsMessage for ${pluginName}:`,
			message,
		);
		const callbacks = this.settingsMessageCallbacks.get(pluginName);
		console.log(
			`[PluginManager] Found ${callbacks?.size || 0} callbacks for ${pluginName}`,
		);
		if (callbacks) {
			for (const callback of callbacks) {
				try {
		...
		for (const [, slate] of this.slates) {
			for (const pattern of slate.config.patterns) {
				...
				if (pattern.startsWith("**/")) {
					...
					if (
						filename === restPattern ||
						filePath.endsWith(`/${restPattern}`)
					) {
						return slate;
					}
				}
	...
	findSlateForFolder(folderPath: string): RegisteredSlate | null {
		for (const [, slate] of this.slates) {
			...
			for (const pattern of slate.config.patterns) {
				const checkPath = require("node:path").join(
					folderPath,
					pattern.replace("**/", ""),
				);
	 ...
	 */
	setSlateWindowMessageHandler(
		handler: (windowId: string, message: unknown) => void,
	): void {
	...
	async mountSlate(
		...
		if (mountHandler) {
			try {
				...
				console.error(
					`[PluginManager] Mount handler failed for slate ${slateId}:`,
					error,
				);
				// Clean up on error
				this.activeSlateInstances.delete(instanceId);
			...
			console.warn(
				`[PluginManager] No mount handler registered for slate ${slateId}`,
			);
		}

		console.info(
			`[PluginManager] Mounted slate instance ${instanceId} for ${filePath}`,
		);
		return instanceId;
	}
	...
	async unmountSlate(instanceId: string): Promise<void> {
		...
		if (!instance) {
			console.warn(
				`[PluginManager] Slate instance not found for unmount: ${instanceId}`,
			);
			return;
		}
	...
	async sendSlateEvent(
		...
		if (!instance) {
			console.warn(
				`[PluginManager] Slate instance not found for event: ${instanceId}`,
			);
			return;
		}
		...
		if (eventHandler) {
			try {
				...
				console.error(
					`[PluginManager] Event handler failed for slate ${instanceId}:`,
					error,
				);
			}
		}
	...
	async activateAllEnabled(): Promise<void> {
		const plugins = Object.values(this.registry.plugins).filter(
			(p) => p.enabled,
		);
