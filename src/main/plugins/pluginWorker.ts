...
let _pluginManifest: PluginManifest | null = null;
...
async function requestFromMain(
	method: string,
	params: unknown,
): Promise<unknown> {
	...
	const api: PluginAPI = {
		...
		workspace: {
			async getWorkspaceFolders(): Promise<WorkspaceFolder[]> {
				return requestFromMain("workspace.getWorkspaceFolders", {}) as Promise<
					WorkspaceFolder[]
				>;
			},

			async readFile(path: string): Promise<string> {
				requirePermission("fs", permissions.fs || "none", [
					"readonly",
					"readwrite",
				]);
				return requestFromMain("workspace.readFile", {
					path,
				}) as Promise<string>;
			},
			...
			async writeFile(path: string, content: string): Promise<void> {
				...
				return requestFromMain("workspace.writeFile", {
					path,
					content,
				}) as Promise<void>;
			},
			...
			async exists(path: string): Promise<boolean> {
				...
				return requestFromMain("workspace.exists", {
					path,
				}) as Promise<boolean>;
			},
			...
			async findFiles(pattern: string): Promise<string[]> {
				...
				return requestFromMain("workspace.findFiles", { pattern }) as Promise<
					string[]
				>;
			},
		},
		...
		editor: {
			async getActiveEditor(): Promise<EditorInfo | null> {
				return requestFromMain(
					...
					{},
				) as Promise<EditorInfo | null>;
			},

			async getSelection(): Promise<string | null> {
				return requestFromMain("editor.getSelection", {}) as Promise<
					string | null
				>;
			},
		...
		terminal: {
			async createTerminal(options: {
				...
				requirePermission("terminal", permissions.terminal || "none", [
					"readonly",
					"readwrite",
				]);
				return requestFromMain(
			...
			async sendText(terminalId: string, text: string): Promise<void> {
				...
				return requestFromMain("terminal.sendText", {
					terminalId,
					text,
				}) as Promise<void>;
			},
		...
		shell: {
			...
			async openExternal(target: string): Promise<void> {
				return requestFromMain("shell.openExternal", {
					target,
				}) as Promise<void>;
			},
		},
		...
		git: {
			async getStatus(repoRoot: string): Promise<unknown> {
				requirePermission("git", permissions.git || "none", [
					"readonly",
					"readwrite",
				]);
			...
			async getBranch(repoRoot: string): Promise<string> {
				...
				return requestFromMain("git.getBranch", {
					repoRoot,
				}) as Promise<string>;
			},
		},
		...
		configuration: {
			async get<T>(key: string): Promise<T | undefined> {
				return requestFromMain("configuration.get", { key }) as Promise<
					T | undefined
				>;
			},

			async update(key: string, value: unknown): Promise<void> {
				return requestFromMain("configuration.update", {
					key,
					value,
				}) as Promise<void>;
			},
		...
		events: {
			...
			onActiveEditorChange(
				callback: (editor: EditorInfo | null) => void,
			): Disposable {
...
self.onmessage = async (
	event: MessageEvent<MainToWorkerMessage & { requestId?: string }>,
) => {
	...
	switch (message.type) {
		case "activate": {
			try {
				...
				_pluginManifest = config.manifest;
		...
		case "command": {
			...
			try {
				...
				const _result = await handler(...(args || []));
