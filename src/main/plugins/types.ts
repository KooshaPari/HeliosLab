...
export function summarizeEntitlements(
	...
	if (entitlements.filesystem) {
		...
		if (fs.fullAccess) {
			summary.push({
				...
				description:
					fs.reason || "Can read and write files anywhere on your system",
			});
		} else if (fs.write) {
			summary.push({
				category: "filesystem",
	...
	if (entitlements.process?.spawn) {
		summary.push({
			...
			label: "Run Processes",
			description:
				entitlements.process.reason ||
				"Can execute shell commands and spawn processes",
		});
	}
	...
	if (entitlements.keybindings) {
		...
		if (kb.global || kb.editor) {
			summary.push({
				...
				description:
					kb.reason ||
					`Registers ${kb.global ? "global" : "editor"} keyboard shortcuts`,
			});
		}
	}
...
export interface PluginAPI {
	...
	slates: {
		...
		onMount(
			slateId: string,
			handler: (context: SlateContext) => void | Promise<void>,
		): Disposable;
		/**
		...
		onUnmount(
			slateId: string,
			handler: (instanceId: string) => void | Promise<void>,
		): Disposable;
		/**
