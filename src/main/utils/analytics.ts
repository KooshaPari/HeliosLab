...
const getAnalyticsConfig = (): AnalyticsConfig => {
	...
	const hasToken =
		MIXPANEL_TOKEN && MIXPANEL_TOKEN !== "BUILD_TIME_MIXPANEL_TOKEN";
...
const shouldTrackEvent = (eventName: string): boolean => {
	...
	if (isAuthenticated()) {
		return (
			config.registered.enabled && config.registered.events.includes(eventName)
		);
	} else {
		return (
			config.community.enabled && config.community.events.includes(eventName)
		);
	}
};
...
export const sendToMixpanel = async (event: string, properties: any) => {
	...
	if (!mixpanel) {
		console.log(
			`Skipping analytics event (no token configured): ${standardizedEvent}`,
		);
		return;
	}
...
export const track = {
	...
	crashReport: (props: { error: string; stack?: string; context?: string }) => {
		...
		const sanitizedStack = props.stack?.replace(
			/\/Users\/[^/]+/g,
			"/Users/***",
		);
...
export const getAnalyticsStatus = () => {
	...
	return {
		...
		eventsTracked: isAuthenticated()
			? config.registered.events
			: config.community.events,
		hasToken: hasToken,
