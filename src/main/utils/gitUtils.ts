...
export const gitValidateUrl = async (gitUrl: string) => {
	try {
		...
		return {
			valid: false,
			error: (error as any)?.message || "Repository not accessible",
		};
	}
};
...
export const gitStashCreate = async (
	...
	try {
		const stashOptions = message
			? ["push", "-m", message, ...options]
			: ["push", ...options];
...
export const getGitConfig = async (): Promise<{
	...
	try {
		...
		try {
			name = (
				await gitInstance.raw(["config", "--global", "user.name"])
			).trim();
		} catch (_e) {
			// Not configured
		}
		...
		try {
			email = (
				await gitInstance.raw(["config", "--global", "user.email"])
			).trim();
		} catch (_e) {
			// Not configured
		}
...
password=${token}
`;
		execSync(`printf '%s' "${input}" | ${OSXKEYCHAIN_HELPER} store`, {
			encoding: "utf-8",
		});
	} catch (error) {
...
export const removeGitHubCredentials = async (): Promise<void> => {
	...
	try {
		...
		const input = `protocol=https\nhost=github.com\n`;
		execSync(`echo "${input}" | ${OSXKEYCHAIN_HELPER} erase`, {
			encoding: "utf-8",
		});
	} catch (error) {
...
export const gitStageHunkFromPatch = async (
	repoRoot: string,
	patch: string,
) => {
...
export const gitStageSpecificLines = async (
	...
	try {
		console.log(
			`Robust git diff-based staging for lines ${startLine}-${endLine} of ${filePath}`,
		);
		...
		for (const hunk of hunks) {
			...
			if (startLine >= hunk.newStart && startLine <= hunkEndLine) {
				targetHunk = hunk;
				console.log(
					`Found target hunk: new lines ${hunk.newStart}-${hunkEndLine}`,
				);
				...
				if (hunk.newCount > 20) {
					...
					const additions = contentChanges.filter((line) =>
						line.startsWith("+"),
					).length;
					const deletions = contentChanges.filter((line) =>
						line.startsWith("-"),
					).length;
		...
		try {
			...
			console.log(
				`Successfully staged hunk containing lines ${startLine}-${endLine}`,
			);
		} catch (_applyError) {
...
export const gitCreateBranch = async (
	...
	try {
		...
		if (!hasCommits) {
			console.log(
				"Repository has no commits, creating initial commit before branch creation...",
			);
			const gitignorePath = path.join(repoRoot, ".gitignore");
			...
			if (status.detached) {
				...
				console.log(
					"Detached HEAD state, creating branch from current HEAD...",
				);
				await gitInstance.checkoutBranch(branchName, "HEAD");
			} else {
...
export const gitTrackRemoteBranch = async (
	...
	try {
		...
		await git(repoRoot).checkoutBranch(
			branchName,
...
export const gitCreatePatchFromLines = async (
	...
	const matchingDeletion = deletions.find((d) => {
		...
		return (
			deletionContent.includes("console.log") &&
			deletionContent.includes("source")
		);
	});
