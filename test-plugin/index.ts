...
import type { Disposable, PluginAPI } from "../colab/src/main/plugins/types";
...
export async function activate(api: PluginAPI): Promise<void> {
	...
	// Enable command
	const zapDisposable = api.commands.registerCommand(
		"electrobun.zap",
		async () => {
	...
	// Disable command
	const restDisposable = api.commands.registerCommand(
		"electrobun.rest",
		async () => {
	...
	// Git status command
	const gitStatusDisposable = api.commands.registerCommand(
		"electrobun.showGitStatus",
		async () => {
			try {
	...
	// Find files command
	const findFilesDisposable = api.commands.registerCommand(
		"electrobun.findFiles",
		async () => {
			try {
	...
	// Shell command demo
	const shellDisposable = api.commands.registerCommand(
		"electrobun.runShell",
		async () => {
			try {
				const result = await api.shell.exec('echo "⚡🐰 Hello from shell!"', {
					timeout: 5000,
				});
	...
	// Open docs command
	const openDocsDisposable = api.commands.registerCommand(
		"electrobun.openDocs",
		async () => {
			...
			if (dirPath) {
				...
				try {
					const result = await api.shell.exec(
						`test -d "${dirPath}" && echo "dir" || echo "file"`,
					);
...
`;
		...
		fields: [
			{
				...
				default: false,
				description:
					"Automatically enable electrobun mode when the plugin loads",
			},
			{
				key: "zapCount",
	...
	const editorChangeDisposable = api.events.onActiveEditorChange((editor) => {
		if (editor) {
			api.log.debug(
				`Active editor changed: ${editor.path} (${editor.languageId})`,
			);
		} else {
	...
	// Handle slate mount
	const slateMountDisposable = api.slates.onMount(
		"bunny-viewer",
		async (context) => {
			...
			const instances =
				api.state.get<Record<string, string>>("slateInstances") || {};
			...
			const fileDir = context.filePath.substring(
				0,
	...
	const slateUnmountDisposable = api.slates.onUnmount(
		"bunny-viewer",
		(instanceId) => {
			...
			if (eventType === "appendToFile") {
				...
				if (filePath) {
					try {
						...
						const _escapedContent = newContent
							.replace(/&/g, "&amp;")
							.replace(/</g, "&lt;")
							.replace(/>/g, "&gt;");
	...
	api.log.info(
		"⚡🐰 Electrobun Demo Plugin activated! All features registered.",
	);
