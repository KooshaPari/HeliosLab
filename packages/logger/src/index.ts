...
class PinoLoggerAdapter implements Logger {
	...
	withLevel(level: LogLevel): Logger {
		return new PinoLoggerAdapter(
			this.inner.child({}, { level: pinoLevel(level) }),
		);
	}
}
...
export class ConsoleLogger implements Logger {
	...
	constructor(
		...
		this._inner =
			Object.keys(baseContext).length > 0 ? base.child(baseContext) : base;
	}
	...
	error(
		...
		this._inner.error(message, error, context);
	}
	fatal(
		message: string,
		error?: Error | unknown,
		...
		this._inner.fatal(message, error, context);
	}
	child(context: Record<string, unknown>): Logger {
		return this._inner.child(context);
	}
	withLevel(level: LogLevel): Logger {
		return this._inner.withLevel(level);
	}
}
