 ...
 */
import { CString, dlopen, type FFIFunction, FFIType } from "bun:ffi";
...
export function nativeStatus(): Record<NativeComponent, NativeLoadState> {
	...
	return {
		...
		persistence: loadState.get("persistence") ?? {
			ok: false,
			reason: "not probed",
		},
		orchestrator: loadState.get("orchestrator") ?? {
			ok: false,
			reason: "not probed",
		},
...
export class PtyPool {
	...
	spawn(options: PtySpawnOptions = {}): number {
		...
		const handle = this.#lib.symbols.pty_pool_spawn(
			shell,
			cwd,
			cols,
			rows,
	...
	write(handle: number, data: Uint8Array | string): number {
		...
		const n = this.#lib.symbols.pty_pool_write(
			handle,
			bytes,
			bytes.byteLength,
		) as number;
...
export class HeliosDb {
	...
	constructor(path: string) {
		...
		if (this.#db === 0)
			throw new Error(`helios_db_open(${path}) returned null`);
	}
...
export class Orchestrator {
	...
	listSessions(): SessionSummary[] {
		const ptr = this.#lib.symbols.HeliosOrchestratorListSessions(
			this.#ptr,
		) as number;
		const raw = this.#takeString(ptr);
...
export class DeviceManager {
	...
	connect(deviceId: string): void {
		const rc = this.#lib.symbols.HeliosDevicesConnect(
			this.#ptr,
			deviceId,
		) as number;
	...
	disconnect(deviceId: string): void {
		const rc = this.#lib.symbols.HeliosDevicesDisconnect(
			this.#ptr,
			deviceId,
		) as number;
	...
	remove(deviceId: string): void {
		const rc = this.#lib.symbols.HeliosDevicesRemove(
			this.#ptr,
			deviceId,
		) as number;
