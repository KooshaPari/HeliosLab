...
describe("Envelope types", () => {
    it("should define CommandEnvelope correctly", () => {
        const cmd: CommandEnvelope = {
            ...
            method: "runtime.runtime.lane.create",
            workspace_id: "ws-1",
            correlation_id: "cor-1",
            payload: { name: "test-lane" },
        };
        expect(cmd.type).toBe("command");
        expect(cmd.method).toBe("runtime.runtime.lane.create");
    });
    ...
    it("should define EventEnvelope correctly", () => {
        const evt: EventEnvelope = {
            ...
            type: "event",
            ts: new Date().toISOString(),
            topic: "runtime.runtime.lane.created",
            lane_id: "lane-1",
            payload: { state: "active" },
        };
        expect(evt.type).toBe("event");
        expect(evt.topic).toBe("runtime.runtime.lane.created");
    });
});
